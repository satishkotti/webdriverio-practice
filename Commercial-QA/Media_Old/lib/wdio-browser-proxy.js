...
function BrowserProxy(browser, options) {
    ...
    //this command will add a redirection for a url or regex expression passed in to the destination url
    _browser.addCommand('redirect', function async(url, destination) {
        var deferred = q.defer();
        var postData = "matchRegex=" + url + "&replace=" + destination;

        proxy.doReq('PUT', '/proxy/' + _port + '/rewrite', postData, function (err, data) {
            if (err) {
                deferred.reject(err);
            } else {
                deferred.resolve('redirected');
            }
        });
        return deferred.promise;
    });

    //this command will return 404 for any urls not in the whitelist, it takes a url regular express
    _browser.addCommand('addWhiteList', function async(url) {
        var deferred = q.defer();
        var postData = "regex=" + url + "&status=404";

        proxy.doReq('PUT', '/proxy/' + _port + '/whitelist', postData, function (err, data) {
            if (err) {
                deferred.reject(err);
            } else {
