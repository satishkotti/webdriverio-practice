...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="sponFlag">
		<xsl:choose>
			<xsl:when test="contains($module_label_2, 'No Sponsor')">
	...
	webmd.fundedEditorial.articleData = {
		...
		<xsl:for-each select="links/link">
			...
			<xsl:choose>
				<xsl:when test="$sponFlag = 'true'">
					<xsl:if test="self::node()[not(contains(string(link_text),'[sp]'))]">
	...
	<xsl:template name="getTOCImgPathNew">
		<xsl:param name="width"/>
		<xsl:param name="height"/>
		<xsl:call-template name="string-replace-all">
			<xsl:with-param name="text">
				<xsl:value-of select="body_images/body_image/source/@path"/>
			</xsl:with-param>
			<xsl:with-param name="replace">
				<xsl:text>650x350</xsl:text>
			</xsl:with-param>
