...
grid1.gettitle=functions.get_Text(grid.grid1articletitle1);
  ...
  adposition: function () {
    ...
    if (browser.isExisting('#webmdHoverClose')) {
      ...
      browser.waitForVisible('//div[@id="ads2-pos-131-rr_add"]', 4000);
      var width = browser.getElementSize('//div[@id="ads2-pos-131-rr_add"]', 'width');
      var height = browser.getElementSize('//div[@id="ads2-pos-131-rr_add"]', 'height');
      var positions = browser.execute(function () {
        return {
          gridposition: $(".list-container").position().left,
          rightadpsoition: $("#ads2-pos-131-rr_add").position().left
        }
      });
