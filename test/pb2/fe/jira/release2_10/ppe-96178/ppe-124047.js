...
describe('PPE-96178: Verify the various conditions set on Template Title input field'
    + 'with respect to the number of characters that the field can accept for a new template', () => {
        ...
        //Tests
                it("When user enters template title which is 64 characters long,"
            + "user must not be displayed with 'Invalid' message for Template Title Input Field", () => {

                //Enter 64 chars long valid text in Page Title field
                _TitleInputField = field.input.get('Template Name ');
                _TitleInputField.setValue(testdata._64CharsLong);

                //Verify the presence of 'Invalid' message field - message should not displayed
                _InvalidMessage = field.invalid.get('Template Name ');
                expect(_InvalidMessage.state).to.equal('failure');
                expect(_InvalidMessage.type).to.equal('NoSuchElement');

            });

        it("When user enters template title which is 65 characters long,"
            + "user must not be displayed with 'Invalid' message for Template Title Input Field", () => {
        ...
        it("When user enters template title which is 66 characters long,"
            + "Page Title input field must contain only 65 characters", () => {
                ...
                expect(GetTheValuePresentInField('Template Name')).to.have.length(65);
