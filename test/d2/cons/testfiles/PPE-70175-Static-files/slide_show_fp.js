...
this.slidelinks_containers[count-1] = sl_cont;
  ...
  this.display = function()
    {
      var s, sl_cont, sl_num, i, hideRegExp, activeRegexp;
	...
	//this.removeParentClassName(this.getElementById(this.linkBaseId + (i+1)).parentNode, 'activeslidelist');
	  ...
	  if (document.getElementById('slide_number')) document.getElementById('slide_number').innerHTML = this.current+1;
	  if (document.getElementById('slide_number_right')) document.getElementById('slide_number_right').innerHTML = this.current+1;
	...
	this.next = function()
    {
	  ...
	  // Register Pageview
	  try {
		var URLFull=String(document.location).split("?"); 
		var URL=URLFull[0];
		var pageNumber = this.current+2;
		if ((this.current+1) == (this.slides.length)) {
			pageURL = URL;
		}
		else {
			pageURL = URL + pageNumber;
		}
  ...
  this.previous = function()
    {
	  ...
	  try {
		...
		if (this.current == 1) {
			...
			if (this.current == 0) {
				pageURL = URL + this.slides.length;
			}
			else {
				pageURL = URL + pageNumber;
			}
